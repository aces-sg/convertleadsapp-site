...
const Login = () => {
  ...
  useEffect(() => {
    if (isBrowser()) {
      ...
      if (rememberedEmail) {
        ...
        setValue('email_update_check', true);
      }
    }
  }, [setValue]);
  ...
  async function onSubmit() {
    let { email, password, email_update_check } = getValues();

    if (isBrowser()) {
      if (email_update_check) {
  ...
  return (
    <AuthLayout>
      <form onSubmit={handleSubmit(onSubmit)} id="loginForm">
        {error && (
          <span className="subTitle text-danger-600">
            {error}
          </span>
        )}
        <div className="">
